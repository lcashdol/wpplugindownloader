...
for x in `curl -sk -A $USERAGENT "https://wordpress.org/plugins/search.php?page=$1&type=term&q=$2" | egrep "href=\"https\:\/\/wordpress.org\/plugins\/[a-z]*|-\""  | awk -F = '{print $2}'  | sed -e 's/"/ /g' | awk '{print $1}' | sort -u |grep plugins`;do
#    echo $x;
if [ $x ]; then
